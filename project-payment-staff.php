...
<head>
    ...
    try{ 
        ...
        $panalty = $db -> query("select sum(panalty) from payments_spend where cid=".$_GET['project']);
        $panalty_res = $panalty->fetchAll();
    ...
    if(!empty($_POST['amount'])){
            ...
            $db -> query("INSERT INTO `payments_spend`(`cid`, `amount`, `panalty`, `yaka_amount`, `yaka_type`, `date`, `avg`, `title`) VALUES ({$_GET["project"]},{$_POST["amount"]},'{$_POST["panalty"]}',{$_POST["yaka_amount"]},'{$_POST["yaka_type"]}','{$_POST["date"]}',{$avg}, '{$_POST["title"]}')");
            $toast = "success";
        ...
        $sum = $db -> query("select sum(avg) from payments_spend where cid=".$_GET['project']);
        $sum_res = $sum->fetchAll();
...
<body class="row justify-content-center">
    ...
    <div class="content justify-content-center row col-md-12">
        ...
        <div class="items col-md-10 row">
            ...
            <div class="col-md-3">
                ...
                <input name="name" <?php echo $edit; ?> type="text" class="form-control form-control-lg form-control form-control-lg-lg" placeholder="<?php echo $res[0][1]; ?>">
            </div>
            <div class="col-md-3">
                <label>Phone number</label>
                <input name="phone" <?php echo $edit; ?> type="text" class="form-control form-control-lg" placeholder="<?php echo $res[0][2]; ?>">
            </div>
            <div class="col-md-3">
                <label>Email</label>
                <input name="email" <?php echo $edit; ?> type="email" class="form-control form-control-lg" placeholder="<?php echo $res[0][3]; ?>">
            </div>
            <div class="col-md-3">
                <label>Address</label>
                <input name="address" <?php echo $edit; ?> type="text" class="form-control form-control-lg" placeholder="<?php echo $res[0][4]; ?>">
            </div>
            <div class="col-md-3">
                <label>Note</label>
                <input name="note" <?php echo $edit; ?> type="text" class="form-control form-control-lg" placeholder="<?php echo $res[0][5]; ?>">
            </div>
            <div class="col-md-3">
                <label>Date</label>
                <input readonly type="text" <?php if(empty($res[0][10])){echo "value='".date('Y-m-d')."'";}else{echo "value='".$res[0][10]."'";} ?> class="form-control form-control-lg">
            </div>
            ...
            <form class="col-md-12" method="post">
                ...
                <div class="row justify-content-center">
                    ...
                    <div class="col-md-2">
                        ...
                        <div class="col-md-2">
                            <label>Panalty</label>
                            <input id="panalty" name="panalty" type="text" class="form-control form-control-lg"
                                placeholder="<?php echo '0'; ?>">
                        </div>
                        <div class="col-md-3">
                            <label>Date</label>
                        ...
                        <div class="m-auto">
                            <button type="submit" class="btn btn-primary p-3">pay money</button>
                        </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="items col-md-10 mt-5">
            <div class="table-list table-striped table-responsive-sm col-md-12">
                <label>List of payed money</label>
                <hr>
                <table class="table table-hover">
                    <thead class="thead-dark">
                        <tr>
                            <th scope="col" id="c1">#</th>
                            <th scope="col" id="c1">Item name</th>
                            <th scope="col" id="c2">Unit Quantity</th>
                            <th scope="col" id="c2">Unit type</th>
                            <th scope="col" id="c2">Unit price</th>
                            <th scope="col" id="c3">Panalty amount</th>
                            <th scope="col" id="c3">Average</th>
                            <th scope="col" id="c3">Remained Money</th>
                ...
                <div class="col-md-12 row">
                    <a class="nav-link btn btn-dark col-md-3 mt-2 p-3 m-auto ml-4" href="project-payment-staff-print.php?part=<?php echo $_GET["part"] ?>&project=<?php echo $_GET["project"] ?>" target="_blank" rel="noopener noreferrer">Print<span class="fas fa-lg fa-print ml-1"></span> <span class="sr-only">(current)</span></a>
                </div>
            </div>
        </div>
        <?php } else{echo "<div class='title col-md-12 mt-5'><div class='items col-md-10 justify-content-center mt-5'>
             Please fill the staff contract first
        </div></div>";}?>
    </div>
    ...
    <div class="toast mr-5 mb-5 p-3" style="position:fixed;bottom:0; right:0;z-index:2;" data-delay="9000" role="alert" aria-live="assertive" aria-atomic="true">
        ...
        <div class="toast-body text-wight-bold">
            ...
            Payment failed, there are not enough money!
            <?php }?>
        </div>
        <script>
