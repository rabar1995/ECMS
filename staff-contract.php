...
<head>
    ...
    if (isset($_FILES["attachment"])){
            ...
            move_uploaded_file($_FILES["attachment"]["tmp_name"],__DIR__.'\\attachments\\staff_contract\\'.$file_name);
            $db -> query("UPDATE staff_contract set attachment='".$file_name."' where cid=".$_GET['project']);
        }
    
    $result = $db -> query("select * from staff_contract where cid=".$_GET['project']);
    $res = $result->fetchAll();
    
    if(!empty($_POST['name']) and empty($res[0][2])){
            $db -> query("INSERT INTO `staff_contract`(`cid`, `name`, `phone`, `email`, `address`, `date`, `note`) VALUES ({$_GET["project"]},'{$_POST["name"]}','{$_POST["phone"]}','{$_POST["email"]}','{$_POST["address"]}','{$_POST["date"]}','{$_POST["note"]}')");
            $success = "true";
        
    }
    if(!empty($_POST["name"]) and !empty($res[0][2])){
        $db -> query("UPDATE `staff_contract` SET `name`='{$_POST["name"]}',`phone`='{$_POST["phone"]}',`email`='{$_POST["email"]}',`address`='{$_POST["address"]}',`date`='{$_POST["date"]}',`note`='{$_POST["note"]}' WHERE cid={$_GET["project"]}");
    }
    if(!empty($_POST["job_type"])){
        $db -> query("INSERT INTO staff_contract_section (pid, job_type, yaka_type, yaka_cost, yaka_amount, payment_type, yaka_avg) values('".$_GET['project']."','".$_POST['job_type']."','".$_POST['yaka_type']."','".$_POST['yaka_cost']."','".$_POST['yaka_amount']."','".$_POST['payment_type']."', ".$_POST['yaka_cost']*$_POST['yaka_amount'].")");
    }
    if(!empty($_GET["delete"])){
        $db -> query("DELETE FROM `staff_contract_section` WHERE id = {$_GET["delete"]}");
    }
    
    $band = $db -> query("select * from staff_band where oid=".$_GET['project']);
    $band_res = $band->fetchAll();
    
    if(!empty($band_res[0][2]) and !empty($_POST["first"])){
        $db -> query("UPDATE `staff_band` SET `first`='{$_POST["first"]}',`second`='{$_POST["second"]}',`third`='{$_POST["third"]}',`forth`='{$_POST["forth"]}',`fifth`='{$_POST["fifth"]}',`sixth`='{$_POST["sixth"]}' WHERE oid = {$_GET["project"]}");
        
    }
    
    if(!empty($_POST["first"]) and empty($band_res[0][2])){
        $db -> query("INSERT INTO staff_band (oid, first, second, third, forth, fifth, sixth) values ({$_GET["project"]}, '{$_POST["first"]}', '{$_POST["second"]}', '{$_POST["third"]}', '{$_POST["forth"]}', '{$_POST["fifth"]}', '{$_POST["sixth"]}')");
...
<body class="row justify-content-center">
        ...
        <div class="items col-md-10 row">
            <div class="row">
                <form method="post" class="row m-auto">
                    <div class="col-md-12">Staff info
                        <hr>
                    </div>
                    <div class="col-md-3">
                        ...
                        <input name="name" type="text"
                            ...
                            value="<?php echo $res[0][1]; ?>" required>
                    </div>
                    <div class="col-md-3">
                        <label>Phone number</label>
                        <input name="phone" type="text" class="form-control form-control-lg"
                            value="<?php echo $res[0][2]; ?>" required>
                    </div>
                    <div class="col-md-3">
                        <label>Email</label>
                        <input name="email" type="email" class="form-control form-control-lg"
                            value="<?php echo $res[0][3]; ?>" required>
                    </div>
                    <div class="col-md-3">
                        <label>Address</label>
                        <input name="address" type="text" class="form-control form-control-lg"
                            value="<?php echo $res[0][4]; ?>" required>
                    </div>
                    <div class="col-md-3">
                        <label>Note</label>
                        <input name="note" type="text" class="form-control form-control-lg"
                            value="<?php echo $res[0][6]; ?>" required>
                    </div>
                    <div class="col-md-3">
                        <label>Date</label>
                        <input type="date" name="date" value="<?= $res[0][5] ?>" class="form-control form-control-lg"
                            required>
                    </div>
                ...
                <form method="post" class="row m-auto">
                    <div id="sections" class="col-md-12 row mt-3">
                        ...
                        <div class="col-md-2">
                            ...
                            <input id="cost" name="yaka_cost" type="text" class="form-control form-control-lg"
                                 required>
                        </div>
                        <div class="col-md-2">
                            <label>Unit Quantity</label>
                            <input id="amount" name="yaka_amount" type="text" class="form-control form-control-lg"
                                placeholder="<?php echo $res[0][8]; ?>" required>
                        </div>
                        <div class="col-md-2">
                            <div class="form-group">
                                ...
                                <select name="payment_type" class="form-control form-control-lg"
                                    ...
                                    <option value="qontarat" <?php if($res[0][13]=="qontarat"){echo "selected";}?>>
                                        Loan</option>
                                </select>
                            </div>
                        </div>
                                ...
                                <script type="text/javascript">
                                    function deleteRow(rowId) {
                                        if (confirm("Are you sure you want to delete this date?")) {
                                            window.location.href = "staff-contract.php?part=<?php echo $_GET["part"]; ?>&project=<?php echo $_GET["project"]; ?>&delete=" + rowId
                                        }
                                    }
            ...
            <form class="row col-md-12" method="post" enctype="multipart/form-data">
                <div class="col-md-12 row">
                    ...
                    <div class="row col-md-3 m-auto">
                        <button type="submit" class="btn btn-primary col-md-12 p-3">Upload attachment <span
                                class="fa fa-upload"></span></button>
                    </div>
                    <?php } else{?>

                    <a class="nav-link btn btn-dark col-md-3 mt-2 p-3 mr-auto ml-4 m-auto"
                        href="attachments/staff_contract/<?php echo $res[0][7]; ?>" download>Download attachment<span
                            class="fa fa-lg fa-download ml-1"></span> <span class="sr-only">(current)</span></a>
                    <?php }?>
                    <a class="nav-link btn btn-dark col-md-3 mt-2 p-3 mr-auto ml-4 m-auto"
                        href="staff-contract-print.php?part=<?php echo $_GET["part"] ?>&project=<?php echo $_GET["project"] ?>"
                        target="_blank" rel="noopener noreferrer">Print contract<span
    ...
    <div class="toast mr-5 mb-5 p-3" style="position:fixed;bottom:0; right:0;z-index:2;" data-delay="9000" role="alert"
        ...
        <div class="toast-body text-wight-bold">
            Staff contract has successfuly updated
        </div>
        <script>
